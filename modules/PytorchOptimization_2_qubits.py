...
class Qubits2Model(nn.Module):
    ...
    def forward(self, T, psi_in):
        ...
        Omega = self.Omega_max * torch.exp(1j * self.phi)
        ...
        # Propagate state using first-order Euler method (differentiable)
        U = torch.eye(self.dim, dtype=torch.complex128, device=device)
        I = torch.eye(self.dim, dtype=torch.complex128, device=device)
        for i in range(self.N):
            U = (I - 1j * H[i] * dt) @ U
        psi_f = U @ psi_in

        # Compute fidelity amplitudes
        a01 = torch.exp(-1j * self.theta) * torch.dot(basis_state[0].conj(), psi_f)
        a11 = torch.exp(-1j * (2 * self.theta + np.pi)) * torch.dot(basis_state[2].conj(), psi_f)
        # Average fidelity
