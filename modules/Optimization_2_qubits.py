...
Omega_max = 100.0   # fix maximum Omega
...
# maybe the number of operations per second is constant
class qubits2_another:
    # input T get optimal 1-F and optimized phi
    def __init__(self, T, psi_in = psi_in, dim = 4, Omega_max = Omega_max, method ='BFGS'):
        self.T = T
        ...
        self.parameter = np.random.uniform(0, 2*np.pi, self.N+1) # Random initial parameters
    
    def refresh(self):
        N0 = self.N
        parameter0 = self.parameter # Store the current parameters
        
        self.N = int(self.T / self.dt) # Update the number of time steps based on the new T
        if self.N != N0:
            self.parameter = np.append(parameter0, np.random.uniform(0, 2*np.pi, self.N - N0)) # Append new random parameters
            print(f"Updated N from {N0} to {self.N}, new parameters length: {len(self.parameter)}")
        else:
            pass
        pass
